-- YouTube Summarizer DB 스키마 업데이트 및 마이그레이션
-- Supabase에서 실행해주세요
...
CREATE TABLE IF NOT EXISTS nicknames (
    ...
    CONSTRAINT unique_nickname_case_insensitive UNIQUE (LOWER(nickname))
);

-- 닉네임 검색을 위한 인덱스 (대소문자 무시)
CREATE INDEX IF NOT EXISTS idx_nicknames_lower ON nicknames(LOWER(nickname));
